...
export function userRoutes(app: Hono<{ Bindings: Env }>) {
    ...
    /**
     * Search for trips based on query parameters
     * GET /api/trips/search?origin=...&destination=...&date=...
     */
    app.get('/api/trips/search', (c) => {
        const { origin, destination, date } = c.req.query();
        if (!origin || !destination) {
            return c.json({ success: false, error: 'Origin and destination are required' }, { status: 400 });
        }
